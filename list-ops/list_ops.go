...
// recoverFunc - allows a function to recover from panic.
func recoverFunc() {
	if r := recover(); r != nil {
		return
	}
...
func (s IntList) Foldl(fn func(int, int) int, initial int) (cal int) {
	...
	defer recoverFunc() // if this func ever panics just return the current value of cal.

	cal = fn(s[0], initial) // reduce the initial val and the first value from list.

	for i := 1; i < sLen; i++ {
		cal = fn(s[i], cal) // reduce subsequent values with cal.
	}
...
func (s IntList) Filter(fn func(int) bool) IntList {
	...
	for _, num := range s { // range over all items in s.
		...
		newS[len] = num
		len++ // update len after adding the item.
	}
