...
func (c shift) Decode(input string) string {
	return shiftCipher(input, c*-1) // we call shiftCipher function with the inverse value of shift.
}
...
func NewVigenere(key ...string) Cipher {
	...
	if lenSlice == 0 {
		return vigenere(genKey()) // return a randomly generated key if no key was provided.
	}
...
func shiftByte(r byte, s shift) byte {
	...
	return shiftCes(getCharPos(r), int(s))
}
